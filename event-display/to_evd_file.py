...
def split_at_timestamp(event,timestamp):
    ...
    if len(indices):
        idx = np.min(indices)
        return event[:idx],event[idx:]
    return event, np.array([], dtype=event.dtype)
...
def main(in_filename, out_filename, *args,
    ...
    if 'pacman_trigger_enabled' in external_trigger_conf and external_trigger_conf['pacman_trigger_enabled']:
        ...
        mask = np.logical_or(mask,sync_packet_mask)
        ...
        for idx, event in zip(event_idx, events[:-1]):
            if idx == 1:
                while len(event_buffer) >= nhit_cut:
                    ...
                    e,event_buffer = split_at_timestamp(event_buffer,min(event_buffer['timestamp'].astype(int))+max_event_dt)
                    evd_file.append(e)
                    event_counter  += 1
                event_buffer = np.array([],dtype=packets.dtype)
            while len(event)+len(event_buffer) >= nhit_cut:
                if len(event_buffer):
                    ...
                    event,event_buffer = split_at_timestamp(event,min(event['timestamp'].astype(int))+max_event_dt)
